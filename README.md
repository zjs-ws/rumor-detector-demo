...
课程结项版本为 **RumorBuster V2.1**。产品版本与内部工作流版本的对应关系如下：

| 产品版本 | 内部工作流 | 状态 |
|---|---|---|
| RumorBuster V2.0 | V2 串行阶段门控图 `rumor_agent_v2` | 保留用于回归比较和故障回退 |
| RumorBuster V2.1 | V3 显式 `StateGraph` `rumor_agent` | 当前默认版本，支持并行取证与确定性裁决 |
...
用户可以在网页端粘贴新闻、社交媒体消息或其他文本进行核验，也可以通过浏览器扩展选中文字并直接进入核验工作区。当前分支已提供 RumorBuster V2.1 网页端、浏览器划词入口、智能体后端、API Gateway、模型调用与本地持久化能力。
...
- RumorBuster V2.1 使用 V3 显式 LangGraph `StateGraph`：通过 `Send` 并行运行本地 RAG、普通网页研究、可选分类器、专业权威研究和默认关闭的确定性时间线研究，以 reducer 汇合后再审查与裁决
- RumorBuster V2.0 的 V2 串行阶段门控图以 `rumor_agent_v2` 保留，可用于回归比较和故障回退
