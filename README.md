...
**RumorBuster V2** 是课程结项后的项目版本名。这里的 `V2` 表示项目归档版本，不等同于代码中为回归兼容而保留的 `rumor_agent_v2` 工作流；当前默认核验流程仍是 V3 显式 `StateGraph`。
...
用户可以在网页端粘贴新闻、社交媒体消息或其他文本进行核验，也可以通过浏览器扩展选中文字并直接进入核验工作区。当前分支已提供 RumorBuster V2 网页端、浏览器划词入口、智能体后端、API Gateway、模型调用与本地持久化能力。
