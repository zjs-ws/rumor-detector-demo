...
def test_prompt_defaults_to_honest_text_only_mode(monkeypatch):
    ...
    assert "已核验谣言知识库（轻量 RAG）：已启用" in rendered
    assert "不能直接决定本轮真假" in rendered
...
def test_prompt_only_advertises_explicitly_enabled_capabilities(monkeypatch):
    ...
    assert "用户消息含有 HTTP(S) URL 时，必须调用一次 `web_fetch`" in rendered
    assert "成功读取正文后必须执行这次独立搜索" in rendered
    assert "原网页不是独立证据" in rendered
    ...
    assert "最后调用一次 `assess_evidence`" in rendered
    assert "禁止自行修改" in rendered
