...
**贡献比例（目标 6:4，最终以工作日志投入时间为准）**：按课程要求依据实际
工作日志/Issue/Commit 和产物填写。当前 git 客观参考：提交数 19:3、改动文件
451:8（上游 7 个基线提交不计）；加上下方剩余任务分配与各自产物后，按日志
投入时间汇总目标落在 **zjswz111 60% : baitea 40%**。日志模板与关键里程碑
骨架见 [docs/WORK_LOG_TEMPLATE.md](WORK_LOG_TEMPLATE.md)。

### 剩余任务分配（8/18–8/20，按 6:4 目标倾斜，不新增 GPU 任务）

| 任务 | 主责 | 产物 |
|---|---|---|
| 30 条真实联网验收执行与失败清单/案例分析 | baitea | 验收结果表、失败分析（07-测试与实验结果） |
| 三组对比评测执行与图表（直接问模型 / 模型+检索 / 完整系统） | baitea | 准确率、引用可追溯率、虚构链接率、延迟等指标表与图表 |
| 三个演示案例跑批与脱敏报告（明确虚假/误导变体/证据不足） | baitea | `evaluation/demo_runs/` 存档与报告 |
| 测试维护与 bug 修复（回归跑批、报告兼容性、前端小修、补充测试用例） | baitea | 修复提交与测试记录 |
| 06-数据集来源说明初稿 | baitea | 训练集/原测试集/Benchmark 来源与冻结说明 |
| 答辩材料整理（模拟答辩记录、PPT 初稿与定稿、视频脚本与剪辑辅助） | baitea | PPT、脚本、答辩记录 |
| 打包检查清单（README-材料说明、目录结构） | baitea | 提交包核对清单 |
| 备用演示预案（无网络静态结果、预录兜底） | zjswz111 | 静态演示材料 |
| 课程报告正文统稿（分工贡献节、三层架构、失败分析） | zjswz111 | 课程报告 .docx/.pdf |
| 答辩 30 秒/2 分钟 QA 卡与两轮模拟答辩组织 | zjswz111 | 问答卡、录音留痕 |
| 最终打包、云盘检查、8/19 缓冲日校对 | zjswz111 | 组长学号压缩包 |
| 模拟答辩、视频录制、报告校对 | 共同 | 视频与校对记录 |

> 核心工作流（V3 状态图、证据规则、信息源）由 zjswz111 主责不变；baitea
> 负责测试维护、bug 修复与答辩材料，均为可核实产出的真实任务。
...
> 统一核验 API、浏览器划词扩展及部署编排。
> 成员二 baitea：负责子智能体执行稳定化、证据-前端衔接与评测材料——诊断并
> 修复子智能体工具执行故障，完成证据字段归一化、前端消息乱序归组与报告
> 卡片防护、构建稳定性改进，并主责 30 条真实联网验收、三组对比评测图表、
> 数据集来源说明与 PPT/视频脚本初稿。
...
> Tavily 结构化抓取、补抓/扫描、扩展与部署（约 60%）
> B（baitea）· 稳定化与评测材料：子智能体执行修复、证据 JSON 容错、
> 消息归组与报告卡片、真实验收与三组对比图表、数据说明与材料初稿（约 40%）

**答辩 1 分钟分工**：

> 项目两人协作：我（A）负责智能体与后端——V3 状态图、证据规则、权威来源
> 注册表、信息源改造与部署；搭档（B）负责子智能体执行稳定化、前端证据
> 展示衔接与评测材料——真实联网验收、三组对比图表和数据来源说明，并共同
> 完成演示案例、报告和模拟答辩。按工作日志投入时间，贡献比约 6:4，具体
> 分工与留痕见小组工作日志。
