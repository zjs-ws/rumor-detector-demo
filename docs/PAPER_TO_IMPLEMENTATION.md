...
本项目没有宣称完整复现三篇论文。数据集与既有划分保持冻结，当前工作是把已经训练完成的 LoRA 三分类模型放入证据优先的系统中，并吸收可验证、可解释的设计思想。
...
| 大语言模型 LoRA 微调的可解释谣言检测相关研究 | 使用参数高效微调适配谣言分类任务，关注标签稳定性与可解释边界 | 使用现有 Qwen2.5-3B 微调权重；严格 `Yes/No/Unknown` 推理；逐子主张审计；与规则结论逐项对照 | 没有重新训练；模型未接受本轮网页证据；训练仅监督标签，因此不把模型生成理由宣传为可信解释 |
